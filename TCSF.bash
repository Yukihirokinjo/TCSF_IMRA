...
sort -k1,1 -k12,12gr -k11,11g  ${out_dir}/Contig_list/Out_BM.txt | sort -u -k1,1 --merge > ${out_dir}/Contig_list/Blast_out_BM.txt
...
seqtk subseq $in_contigs ${out_dir}/Contig_list/TCSF_list.txt > ${out_dir}/TCSF_contigs_${out_dir}.fna

		if [ "$?" -ne 0 ] || [ ! -s "${out_dir}/TCSF_contigs_${out_dir}.fna" ]; then
...
printf  "\n ---------------------------- Run Evalation\n"
  ...
  Rscript --vanilla --slave  `which AssemblyEval.R`  ${out_dir}  Initial  ${out_dir}/TCSF_contigs_${out_dir}.fna

  cat ${out_dir}/tmp_Result.log >> ${out_dir}/Result.log
  rm  ${out_dir}/tmp_Result.log


#1> /dev/null	quast.py -o  ${out_dir}/quast  ${out_dir}/TCSF_contigs_${out_dir}.fna
#
#			if [ "$?" -ne 0 ]; then
#				echo "[Error] Quast failed. Please check the Messages avobe" 1>&2
#				exit 1
#			fi
