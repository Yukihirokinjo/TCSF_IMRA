...
-> Description above is not necessary in current version (ver. 2.5.3). It automatically recognizes and converts the fastq type.
...
##### [TCSF]
```
$ TCSF-2.5.3.bash  -i input_contigs.fasta -refB reference_bacterial_genome.fasta
```

For closely related genome (use blastn)
```
$ TCSF-2.5.3.bash  -i input_contigs.fasta -refB reference_bacterial_genome.fasta -blaN
```


##### [IMRA]
```
$ IMRA-2.5.3.bash  -ref TCSF_contigs.fasta -inF input_read_1.fastq -inR input_read_2.fastq -spa
```

For >150bp reads
```
$ IMRA-2.5.3.bash  -ref TCSF_contigs.fasta -inF input_read_1.fastq -inR input_read_2.fastq -spa -k 33,55,91,121
```

For the use of reference contigs/scaffolds from a genome of closely related strain
```
$ IMRA-2.5.3.bash  -ref ref_contigs.fasta -inF input_read_1.fastq -inR input_read_2.fastq -spa -sensitive
```
