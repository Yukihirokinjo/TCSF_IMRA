...
usage_exit() {
	echo "Usage: IMRA.bash [-ref Reference_Contigs] [-inF input_Reads_1 ] [-inR input_Reads_2] < Options.. >" 1>&2
	exit 1
}
...
[ -z "$out_dir" ] && out_dir="IMRA_OUT_`date +%Y%m%d`_`date +%H%M%S`"
...
# one correctly mapped: 
  ...
  #Check read type 
  CASAVAold=`grep -c "#0" ${out_dir}/IDs/selected_ID_${num}.txt` 
  if [ "${CASAVAold}" == "0" ]; then
    CASAVAtype="new"
...
##---------------------------------------------------------------Optionally
#Output large information files
#  if [ -z $info_M ]; then
#    rm ${out_dir}/Map/${num}/454*Status.txt
#    rm ${out_dir}/Map/${num}/454PairAlign.txt
#  fi
